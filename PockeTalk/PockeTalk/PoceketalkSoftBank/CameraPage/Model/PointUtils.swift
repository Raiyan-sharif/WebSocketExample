...
public class PointUtils {
    static func parseResponseForBlock( dataToParse: Full_Text_Annotation!, mDetectedLanguageCode: String, xFactor: Float, yFactor: Float, completion: @escaping (_ result: BlockClass) -> Void){
        var blockClass: BlockClass = BlockClass(languageCodeFrom: "", blocks: [])
        
        PrintUtility.printLog(tag: "Point Utils", text: "xFactor \(xFactor), yFactor: \(yFactor)")
...
//                        }
                        ...
                        if (detectedLanCode == LANGUAGE_CODE_UND || detectedLanCode == CHINESE_LANGUAGE_CODE_ZH) {
                            LanguageDetection().getDetectedLanguage(with: blockText) { result in
                                if let langCode = result {
                                    detectedLanCode = langCode
                                    let blockElement = BlockElement(boundingBox: blockBoundingBoxes, bottomTopBlock: bottomTopBlock, rightLeftBlock: rightLeftBlock, text: blockText, detectedLanguage: detectedLanCode)
            ...
            completion(blockClass)
        }

        //PrintUtility.printLog(tag: " data parse.text ", text: "\(dataToParse.text)")
    }
    
    static func parseResponseForLine( dataToParse: Full_Text_Annotation!, mDetectedLanguageCode: String, xFactor: Float, yFactor: Float, completion: @escaping (_ result: BlockClass) -> Void) {
        var blockClass: BlockClass = BlockClass(languageCodeFrom: "", blocks: [])
